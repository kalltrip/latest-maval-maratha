...
document.addEventListener("DOMContentLoaded", function () {
    ...
    .then(([vegGravy, nonVegGravy]) => {
        ...
        //  Add click handlers to all dish images after populating menus set timeout to 5secs
        setTimeout(addDishClickHandlers, 5000);
    })
    .catch(error => console.error("Error fetching gravy data:", error));
});
...
// Replace the existing addDishClickHandlers function with this:
function addDishClickHandlers() {
    ...
    dishImages.forEach(img => {
        ...
        img.removeEventListener('click', handleDishImageClick);
        
        img.style.cursor = 'pointer';
        // Add the event listener with a named function
        img.addEventListener('click', handleDishImageClick);
    });
}

// Function to add click handlers to all dish images
function handleDishImageClick(event) {
