...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const chineseVegContainer = document.getElementById("chinese-veg-menu");

    if (!chineseVegContainer) {
        console.error("Chinese menu container not found on page. Check your HTML.");
        return;
    }

    // Fallback data in case JSON loading fails
    const fallbackChineseVegData = [
        {
            name: "Vegetable Spring Rolls",
            price: 120,
            description: "Crispy spring rolls filled with mixed vegetables.",
            image: "../images/chinese/veg-spring-rolls.jpg",
            rating: 4.3,
            veg: true
        },
        {
    ...
    // Fetch both JSON files in parallel
    Promise.all([
        fetch("../json/chinese_veg.json")
            .then(response => {
                if (!response.ok) {
                    throw new Error(`Failed to load chinese_veg.json: ${response.status}`);
                }
    ...
    .catch(error => {
        console.error("Error fetching chinese data:", error);
        // Use fallback data if fetch fails
        console.log("Using fallback chinese vegetarian data");
        populateChineseMenu(fallbackChineseVegData, "chinese-veg-menu");
...
function findRelatedDishes(isVeg, currentDishName) {
    const relatedDishes = [];
    const allDishes = document.querySelectorAll('#chinese-veg-menu .dish');
