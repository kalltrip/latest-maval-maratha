...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const beveragesContainer = document.getElementById("beverages-menu");

    if (!beveragesContainer) {
        console.error("Beverages menu container not found on page. Check your HTML.");
        return;
    }

    // Fallback data in case JSON loading fails
    const fallbackBeveragesData = [
        {
            name: "Masala Chai",
            price: 30,
        ...
        },
        {
            name: "Lassi",
            price: 50,
            description: "Traditional yogurt-based sweet or salty drink.",
        ...
        },
        {
            name: "Butter Milk",
            price: 40,
            description: "Refreshing spiced buttermilk with herbs.",
            image: "../images/beverages/butter-milk.jpg",
            rating: 4.3,
            veg: true
        }
    ];
    ...
    fetch("../json/beverages.json")
        ...
        .catch(error => {
            console.error("Error fetching beverages.json:", error);
            // Use fallback data if fetch fails
            console.log("Using fallback beverages data");
            populateBeveragesMenu(fallbackBeveragesData);
    ...
    if (relatedDishes.length > 0) {
        relatedDishesHTML = `
            <div class="related-dishes">
                <h3>Related Beverage Options</h3>
...
// Function to find related beverages
function findRelatedDishes(isVeg, currentDishName) {
    const relatedDishes = [];
    const allDishes = document.querySelectorAll('#beverages-menu .dish');
