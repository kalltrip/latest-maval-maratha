...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const vegContainer = document.getElementById("handi-veg-menu");
    const nonVegContainer = document.getElementById("handi-nonveg-menu");

    if (!vegContainer || !nonVegContainer) {
        console.error("One or both handi menu containers not found on page. Check your HTML.");
        return;
    }
    ...
    const fallbackVegData = [
        {
            name: "Paneer Handi",
            price: 220,
            description: "Cottage cheese cubes cooked in a rich and creamy gravy with bell peppers and spices.",
            image: "../images/handi/paneer-handi.jpg",
            rating: 4.5,
            veg: true
        },
        {
            name: "Veg Kadai Handi",
            price: 200,
            description: "Mixed vegetables cooked in a spicy kadai masala with bell peppers and onions.",
            image: "../images/handi/veg-kadai-handi.jpg",
            rating: 4.3,
            veg: true
        },
        {
            name: "Mushroom Handi",
            price: 210,
            description: "Fresh mushrooms cooked in a creamy gravy with aromatic spices.",
            image: "../images/handi/mushroom-handi.jpg",
            rating: 4.2,
            veg: true
        }
    ];
    ...
    const fallbackNonVegData = [
        {
            ...
            image: "../images/handi/butter-chicken-handi.jpg",
            rating: 4.7,
            veg: false
        },
        {
            name: "Chicken Kadai Handi",
            price: 240,
            description: "Chicken cooked with bell peppers and onions in a spicy kadai masala.",
            image: "../images/handi/chicken-kadai-handi.jpg",
            rating: 4.6,
            veg: false
        },
        {
            name: "Mutton Rogan Josh Handi",
            price: 280,
            description: "Tender mutton pieces cooked in aromatic Kashmiri spices.",
            image: "../images/handi/mutton-rogan-josh-handi.jpg",
            rating: 4.8,
            veg: false
        }
    ];
    ...
    // Fetch both JSON files in parallel
    Promise.all([
        fetch("../json/handi_veg.json")
            .then(response => {
                if (!response.ok) {
                    throw new Error(`Failed to load handi_veg.json: ${response.status}`);
                }
                return response.json();
            })
            .catch(error => {
                console.error("Error fetching handi_veg.json:", error);
                console.log("Using fallback vegetarian handi data");
                return fallbackVegData;
            }),
        fetch("../json/handi_non_veg.json")
            .then(response => {
                if (!response.ok) {
                    throw new Error(`Failed to load handi_non_veg.json: ${response.status}`);
                }
                return response.json();
            })
            .catch(error => {
                console.error("Error fetching handi_non_veg.json:", error);
                console.log("Using fallback non-vegetarian handi data");
    ...
    .catch(error => console.error("Error processing handi data:", error));
});
...
function findRelatedDishes(isVeg, currentDishName) {
    const relatedDishes = [];
    // Get dishes from the appropriate menu based on vegetarian status
    const menuId = isVeg ? 'handi-veg-menu' : 'handi-nonveg-menu';
    const allDishes = document.querySelectorAll(`#${menuId} .dish`);
    ...
    allDishes.forEach(dishElement => {
        ...
        // Only include dishes with the same vegetarian status (should always be true given our selection)
